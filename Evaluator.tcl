...
proc evaluate_usage { } {
  ...
  vmdcon -info "  -WBS <0/1>      Enable WBS analysis    (default: 0)"
  error ""
...
proc evaluate_core {args} {
  ...
  # Set some defaults
    ...
    array set options {
      ...
      -cal "type OT"           -r 3.5
      -RDF 0      -ODF 0       -HB 0     -MSD 0     -MRT 0     -WBS 0
    ...
    foreach {key validator} {
      ...
      -MRT    validate_switch -WBS     validate_switch
  ...
  # Start counting
    ...
    dumpscripts $psffile $step $options(-n) "$options(-ref)" "$options(-cal)" $options(-r) $options(-RDF) $options(-ODF) $options(-HB) $options(-MSD) $options(-MRT) $options(-WBS)
  ...
  # Check and integrate
    ...
    foreach analysis {RDF ODF HB MSD MRT WBS} {
...
proc dumpscripts {psffile step n refatm calatm r RDF ODF HB MSD MRT WBS} {
  ...
  for {set i 0} {$i < $n} {incr i} {
    ...
    if { $WBS } {puts $script "measure_WBS Data_WBS_$i \"$refatm\" \"$calatm\" $r"}
...
proc measure_RDF {outputname refatm calatm {dr "0.1"} {rmax "10.0"}} {
...
proc measure_ODF {outputname refatm calatm rmax} {
...
proc measure_HB {outputname refatm calatm rmax {amax "30"}} {
...
proc measure_MSD {outputname refatm calatm rmax} {
...
proc measure_MRT {outputname refatm calatm rmax} {
...
proc measure_WBS {outputname refatm calatm rmax {amax 30}} {
  ...
  foreach index [$refsel get index] {
    ...
    if {[expr floor([$sel get mass])] == 1} {
        ...
        for {set i 1} {$i < [expr $nframe - 100]} {incr i} {
            ...
            set sel_water [atomselect top "$calatm && pbwithin $rmax of index $index"]
            $sel_water frame $i
            $sel_water update
            foreach j [$sel_water get index] {

                set sel_OT [atomselect top "index $j"]
                ...
                if {[llength [lindex [measure hbonds $rmax $amax $sel $sel_OT] 0]] != 0} {
                    ...
                    if {[llength [lindex [measure hbonds $rmax $amax $sel $sel_OT] 0]] != 0} {
                        ...
                        for {set n $i} {$n <= [expr $i + 50]} {incr n} {
                            ...
                            if {[llength [lindex [measure hbonds $rmax $amax $sel $sel_OT] 0]] == 0 || $found} {
                        ...
                        puts $fileid "$index $j $i [join $mark " "]"
                ...
                if {[llength [lindex [measure hbonds $rmax $amax $sel_OT $sel] 0]] != 0} {
                    ...
                    if {[llength [lindex [measure hbonds $rmax $amax $sel_OT $sel] 0]] != 0} {
                        ...
                        for {set n $i} {$n <= [expr $i + 50]} {incr n} {
                            ...
                            if {[llength [lindex [measure hbonds $rmax $amax $sel_OT $sel] 0]] == 0 || $found} {
...
proc integrate_WBS {n {tcut 5}} {

    set avg_map [dict create]
    set count_map [dict create]
    set file_bound [open "Data_boundwater.dat" w]

    for {set i 0} {$i < $n} {incr i} {

        set filename "Data_WBS_$i.dat"
        set file [open $filename r]

        while {[gets $file line] >= 0} {

            set fields [split $line]
            if {[llength $fields] != 54} continue

            set target_atom [lindex $fields 0]
            set water_atom [lindex $fields 1]
            set start_frame [lindex $fields 2]
            set marklist [lrange $fields 3 end]
            set marklen [llength $marklist]

            if {$marklen >= $tcut && [lrange $marklist 0 [expr {$tcut-1}]] eq [lrepeat $tcut "1"]} {

                set life $tcut
                for {set j $tcut} {$j < $marklen} {incr j} {
                    if {[lindex $marklist $j] ne "1"} break
                    incr life
                }
                puts $file_bound "$target_atom $water_atom $i $start_frame $life"
            }

            if {![dict exists $avg_map $target_atom]} {
                dict set avg_map $target_atom [lrepeat $marklen 0]
                dict set count_map $target_atom 0
            }

            set current [dict get $avg_map $target_atom]
            for {set j 0} {$j < $marklen} {incr j} {
                set val [expr {[lindex $current $j] + [lindex $marklist $j]}]
                lset current $j $val
            }
            dict set avg_map $target_atom $current
            dict incr count_map $target_atom
        }
        close $file
    }
    close $file_bound

    set file_avg [open "Data_WBS.dat" w]
    
    dict for {key sumlist} $avg_map {
        set count [dict get $count_map $key]

        if {$count == 0} {
            set avglist [lrepeat [llength $sumlist] 0.0]
        } else {
            set avglist {}
            foreach val $sumlist {
                lappend avglist [format "%.3f" [expr {double($val) / $count}]]
            }
        }
        puts $file_avg "$key [join $avglist " "]"
    }
    close $file_avg
}
